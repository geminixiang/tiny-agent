...
test("bg manages lifecycle, logs, fast failures, and stale metadata", async () => {
    ...
    assert.ok(
        JSON.parse(await executeTool("bg", { action: "list", status: "stale" })).some(
            (meta: any) => meta.id === started.id,
        ),
    );
    ...
    assert.deepEqual(JSON.parse(await executeTool("bg", { action: "list" })), []);
    assert.ok(
        JSON.parse(await executeTool("bg", { action: "list", status: "all" })).some(
            (meta: any) => meta.id === started.id,
        ),
    );
    ...
    assert.deepEqual(JSON.parse(await executeTool("bg", { action: "list" })), []);
    assert.ok(
        JSON.parse(await executeTool("bg", { action: "list", status: "exited" })).some(
            (meta: any) => meta.id === failedMeta.id,
        ),
    );
