...
test("projects canonical lifecycle events into startup and operation span trees", async () => {
    ...
    assert.equal(startup.attributes["gen_ai.request.model"], undefined);
    assert.equal(operation.attributes["openinference.span.kind"], "AGENT");
    assert.equal(operation.attributes["gen_ai.operation.name"], "invoke_agent");
    assert.equal(operation.attributes["gen_ai.conversation.id"], sessionId);
    assert.equal(operation.attributes["session.id"], sessionId);
    ...
    assert.equal(model.attributes["openinference.span.kind"], "LLM");
    assert.equal(model.attributes["gen_ai.operation.name"], "chat");
    assert.equal(model.attributes["gen_ai.request.model"], "test-model");
    assert.equal(model.attributes["llm.model_name"], "test-model");
    assert.equal(model.attributes["gen_ai.usage.input_tokens"], 11);
    assert.equal(model.attributes["gen_ai.usage.cache_read.input_tokens"], 3);
    assert.equal(model.attributes["gen_ai.usage.cache_creation.input_tokens"], 1);
    assert.equal(model.attributes["llm.token_count.prompt"], 11);
    assert.equal(model.attributes["llm.token_count.completion"], 2);
    assert.equal(model.attributes["llm.token_count.total"], 13);
    assert.equal(model.attributes["tiny.usage.input_tokens"], 7);
    ...
    assert.equal(tool.attributes["session.id"], sessionId);
