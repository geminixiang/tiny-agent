...
const themeAsset = generatedAssets.find((path) => /^\/assets\/theme\.[a-f0-9]{12}\.js$/.test(path));
...
test("build emits Cloudflare Pages files and content-hashed assets", async () => {
    ...
    assert.ok(generatedAssets.length >= 8);
    ...
    for (const html of pages.values()) {
        ...
        assert.match(html, new RegExp(`<script src="${themeAsset}"></script>[^]*?<link rel="stylesheet"`));
