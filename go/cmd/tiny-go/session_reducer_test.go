...
func TestSessionGoldenFixtures(t *testing.T) {
	root, err := findRepositoryRoot()
	if err != nil {
		t.Fatal(err)
	}
	fixtures := filepath.Join(root, "schemas", "session", "fixtures")
	var manifest struct {
		Fixtures []struct {
	...
	for _, fixture := range manifest.Fixtures {
		t.Run(fixture.Name, func(t *testing.T) {
			...
			var expected struct {
				...
				Error *sessionCorruption `json:"error"`
			}
			readJSONFile(t, filepath.Join(fixtures, fixture.Expected), &expected)
			state, reduceErr := reduceSession(data)
			if expected.OK {
				if reduceErr != nil {
					t.Fatal(reduceErr)
				}
			...
			var corruption *sessionCorruption
...
func findRepositoryRoot() (string, error) {
	...
	for {
		if _, err := os.Stat(filepath.Join(dir, "schemas", "session", "fixtures", "manifest.json")); err == nil {
			return dir, nil
		}
