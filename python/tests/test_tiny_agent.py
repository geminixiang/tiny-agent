...
JSON_LIFECYCLE_CONTRACT = json.loads((Path(__file__).resolve().parents[2] / "schemas/monitoring/execution-lifecycle-contract.json").read_text(encoding="utf-8"))
BASH_CONTRACT = json.loads((Path(__file__).resolve().parents[2] / "schemas/tools/bash-contract.json").read_text(encoding="utf-8"))
...
class TinyAgentTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.old_root = tiny.ROOT
        tiny.ROOT = Path(self.temp.name).resolve()
        self.addCleanup(self.temp.cleanup)
        self.addCleanup(setattr, tiny, "ROOT", self.old_root)
    ...
    async def test_tools_paths_and_large_bash_output(self):
        ...
        result = await tiny.execute_tool("bash", {"command": "printf begin; yes x | head -n 30000; printf end"})
        path = Path(re.search(r"Full output: (.*\.log)\]", result).group(1))
        self.assertTrue(path.read_text(encoding="utf-8").startswith("begin"))
        self.assertTrue(path.read_text(encoding="utf-8").endswith("end"))
    ...
    async def test_shared_bash_contract(self):
        for scenario in BASH_CONTRACT["cases"]:
            ...
            if "timeout" in scenario:
                arguments["timeout"] = scenario["timeout"]
    ...
    async def test_bash_error_preserves_output_and_cancel_kills_children(self):
        ...
        old_limit = tiny.MAX_BASH_OUTPUT
        tiny.MAX_BASH_OUTPUT = 1024
        try:
            ...
            tiny.MAX_BASH_OUTPUT = old_limit
    ...
    async def test_bash_monitors_eof_cancellation_and_total_timeout(self):
        ...
        task = asyncio.create_task(
            tiny.execute_bash(
                f"echo $$ > {cancel_pid}; exec 1>&-; sleep 30",
                cancelled,
            )
        )
        ...
        with patch.object(tiny, "BASH_TIMEOUT_SECONDS", 0.15):
            ...
            self.assertEqual(
                await asyncio.wait_for(
                    tiny.execute_bash(
                        f"echo $$ > {timeout_pid}; exec 1>&-; sleep 30",
                        asyncio.Event(),
                    ),
                    1,
                ),
    ...
    async def test_bash_success_kills_redirected_background_descendant(self):
        ...
        self.assertEqual(
            await tiny.execute_bash(
                f"echo $$ > {group_path}; sleep 30 </dev/null >/dev/null 2>&1 & echo $! > {child_path}; exit 0",
                asyncio.Event(),
            ),
            "(no output)",
        )
    ...
    async def test_chunked_openrouter_json_and_event_cancellation(self):
        ...
        connected.clear()
        server = await asyncio.start_server(quiet, "127.0.0.1", 0)
        try:
            port = server.sockets[0].getsockname()[1]
            cancelled = asyncio.Event()
            ...
            with self.assertRaisesRegex(InterruptedError, "aborted"):
                await asyncio.wait_for(task, 1)
            await asyncio.sleep(0)
            self.assertTrue(all(writer.is_closing() for writer in writers))
        finally:
            server.close()
            await server.wait_closed()
    ...
    async def test_skills_project_instructions_and_prompt(self):
        ...
        path = tiny.ROOT / ".tiny-agent/skills/hello/SKILL.md"
        ...
        self.assertIn("Always be brief.", system)
    ...
    async def test_session_shape_and_idle_resume(self):
        ...
        restored = tiny.Agent(session=reopened)
        await restored.resume_session()
    ...
    def recovery_agent(self, fixture: str, replies=()):
        ...
        session_id = state["header"]["id"]
        directory = tiny.ROOT / ".tiny-agent/sessions"
    ...
    async def test_recovers_open_attempt_and_is_idempotent(self):
        ...
        with patch.dict(os.environ, {"TINY_AGENT_ENVIRONMENT_IDENTITY": "fixture"}):
            ...
            self.assertIsNone(await agent.resume_session())
    ...
    async def test_recovers_abort_and_blocks_without_writes(self):
        with patch.dict(os.environ, {"TINY_AGENT_ENVIRONMENT_IDENTITY": "fixture"}):
            ...
            self.assertIsNone(await aborted.resume_session())
            self.assertEqual(aborted_session.load()["operation"]["kind"], "idle")
            ...
            with self.assertRaisesRegex(RuntimeError, "attempts_exhausted"):
                await exhausted.resume_session()
            self.assertEqual(exhausted_session.path.read_bytes(), before)
            exhausted_session.close()

            mismatch, mismatch_session = self.recovery_agent("open-attempt.jsonl")
            mismatch.configuration_digest = "sha256:" + "f" * 64
            before = mismatch_session.path.read_bytes()
            with self.assertRaisesRegex(RuntimeError, "configuration_changed"):
                await mismatch.resume_session()
            self.assertEqual(mismatch_session.path.read_bytes(), before)
            mismatch_session.close()

    async def _completed_conversation(self, session, turns=4):
        replies = iter(
            [{"choices": [{"message": {"role": "assistant", "content": f"answer {index}"}, "finish_reason": "stop"}], "usage": {"prompt_tokens": 10, "completion_tokens": 2}} for index in range(turns)]
        )
        agent = tiny.Agent(session=session, requester=lambda *_: async_value(next(replies)))
        for index in range(turns):
            self.assertEqual(await agent.run_agent_loop(f"question {index}"), f"answer {index}")
        return agent

    async def test_durable_compaction_and_idle_resume(self):
        session = tiny.Session.create(tiny.ROOT)
        agent = await self._completed_conversation(session)
        agent.requester = lambda *_: async_value(
            {"choices": [{"message": {"role": "assistant", "content": "durable summary"}, "finish_reason": "stop"}], "usage": {"prompt_tokens": 20, "completion_tokens": 3}}
        )
        ...
        restored = tiny.Agent(session=reopened)
        await restored.resume_session()
        self.assertEqual(restored.messages[1:], state["activeContext"])
        reopened.close()
    ...
    async def test_repeated_compaction_uses_bounded_active_context(self):
        ...
        for index in range(4):
            await agent.run_agent_loop(f"new question {index}")
        agent.requester = compact_reply
        await agent.compact()
    ...
    async def test_recovered_compaction_abort_race_records_known_usage(self):
        ...
        session.append(
            {
                ...
                "record": {
                    ...
                    "inputThroughEntryId": source[-1]["id"],
                    "resultEntryId": result_id,
                    "compactedEntryIds": [item["id"] for item in source[:2]],
                    "retainedEntryIds": [item["id"] for item in source[2:]],
        ...
        agent = tiny.Agent(session=session, requester=reply)
        agent.configuration_digest = agent.configuration_digest
    ...
    async def test_recovers_open_and_committed_compaction_idempotently(self):
        ...
        state = session.load()
        source = agent._message_facts()
        input_id = source[-1]["id"]
        compacted, retained = source[:2], source[2:]
        operation_id, result_id = tiny.uuid7(), tiny.uuid7()
        session.append(
            {
                "kind": "record",
        ...
        session_id = session.id
        session.close()
        blocked = tiny.Session.open(session_id, tiny.ROOT)
        mismatch = tiny.Agent(session=blocked)
        before = blocked.path.read_bytes()
        mismatch.configuration_digest = "sha256:" + "f" * 64
        with self.assertRaisesRegex(RuntimeError, "configuration_changed"):
            await mismatch.resume_session()
        self.assertEqual(blocked.path.read_bytes(), before)
        blocked.close()

        reopened = tiny.Session.open(session_id, tiny.ROOT)
        recovered = tiny.Agent(
            session=reopened, requester=lambda *_: async_value({"choices": [{"message": {"role": "assistant", "content": "recovered summary"}, "finish_reason": "stop"}], "usage": {}})
        )
        recovered.configuration = agent.configuration
        recovered.configuration_digest = agent.configuration_digest
        await recovered.resume_session()
        self.assertEqual(reopened.load()["operation"]["kind"], "idle")
        before = reopened.path.read_bytes()
        self.assertIsNone(await recovered.resume_session())
        ...
        unfinished = tiny.Session.open(session_id, tiny.ROOT)
        finisher = tiny.Agent(session=unfinished)
        finisher.configuration = agent.configuration
        finisher.configuration_digest = agent.configuration_digest
        await finisher.resume_session()
        self.assertEqual(unfinished.load()["operation"]["kind"], "idle")
        finished = unfinished.path.read_bytes()
        self.assertIsNone(await finisher.resume_session())
        self.assertEqual(unfinished.path.read_bytes(), finished)
        unfinished.close()
    ...
    async def test_model_tool_loop_cache_and_no_tui_log_in_session(self):
                    ...
                    "choices": [
                        {
                            "message": {
                                "role": "assistant",
                                "content": None,
                                "tool_calls": [{"id": "1", "type": "function", "function": {"name": "write", "arguments": '{"path":"made.txt","content":"yes"}'}}],
                            }
                        }
                    ],
                    "usage": {"prompt_tokens": 100, "completion_tokens": 10, "prompt_tokens_details": {"cached_tokens": 25}},
                },
                {"choices": [{"message": {"role": "assistant", "content": "done"}}], "usage": {"prompt_tokens": 120, "completion_tokens": 5, "prompt_cache_hit_tokens": 60}},
            ]
        )
        requests, events = [], []

        async def request(body, _):
            requests.append(body)
            return next(replies)

        session = tiny.Session.create(tiny.ROOT)
        agent = tiny.Agent(session=session, requester=request, on_tool=events.append)
        ...
        self.assertFalse(
            any(fact.get("kind") == "tool_log" for line in session.path.read_text().splitlines()[1:] for fact in (json.loads(line) if isinstance(json.loads(line), list) else [json.loads(line)]))
        )
    ...
    async def assert_aborted_reopens_idempotently(self, session, expected, usage=None):
        ...
        restored = tiny.Agent(session=reopened)
        before = reopened.path.read_bytes()
        self.assertIsNone(await restored.resume_session())
        self.assertEqual(reopened.path.read_bytes(), before)
        self.assertEqual(restored.messages[-1].get("content"), expected)
        if usage:
            for key, value in usage.items():
                self.assertEqual(restored.usage[key], value)
        reopened.close()
    ...
    async def test_abort_after_successful_model_return_discards_answer(self):
        ...
        self.assertEqual(agent.usage["input"], 8)
        self.assertEqual(agent.usage["output"], 2)
        self.assertEqual(session.load()["usage"]["input"], 8)
        self.assertEqual(session.load()["usage"]["output"], 2)
    ...
    async def test_abort_race_after_model_settlement_does_not_create_durable_abort(self):
        ...
        session.close()
        reopened = tiny.Session.open(session_id, tiny.ROOT)
        self.assertIsNone(await tiny.Agent(session=reopened).resume_session())
        reopened.close()
    ...
    async def test_abort_after_successful_tool_return_materializes_interruption(self):
        ...
        response = {
            "choices": [
                {
                    "message": {"role": "assistant", "content": None, "tool_calls": [{"id": "race-call", "type": "function", "function": {"name": "race", "arguments": "{}"}}]},
                    "finish_reason": "tool_calls",
                }
            ],
            "usage": {},
    ...
    async def test_abort_after_successful_compaction_return_discards_summary(self):
        ...
        self.assertEqual(session.load()["usage"]["input"], 53)
        self.assertEqual(session.load()["usage"]["output"], 13)
            ...
            [
                {
                    "choices": [
                        {
                            "message": {"role": "assistant", "content": None, "tool_calls": [{"id": "read-call", "type": "function", "function": {"name": "read", "arguments": '{"path":"missing"}'}}]},
                            "finish_reason": "tool_calls",
                        }
                    ],
                    "usage": {},
    ...
    async def test_cli_displays_recovery_through_terminal_before_continuation(self):
        ...
        class FakeAgent:
            def __init__(self, *_args, **_kwargs):
                self.usage = {"input": 7, "output": 3, "cacheRead": 0, "cacheWrite": 0}
                self.on_tool = None
            ...
            async def run_agent_loop(self, text):
                calls.append(f"run:{text}")
        ...
        class FakeTerminal:
            ...
            async def run(self, _agent, operation):
                calls.append("terminal.run")
                return await operation()

        output = io.StringIO()
        with (
            patch.object(cli, "load_mcp_configs", return_value=[]),
            ...
            patch.object(cli, "Agent", FakeAgent),
            patch.object(cli, "Terminal", FakeTerminal),
        ...
        class FakeAgent:
            ...
            async def run_agent_loop(self, _text):
                ...
                self.lifecycle.committed(
                    [{"kind": "record", "timestamp": 2, "record": {"type": "stepAttempt", "operationId": "operation-1", "stepId": "step-1", "attemptId": "attempt-1", "attempt": 1}}]
                )
                self.lifecycle.committed(
                    [
                    ...
                    [
                        {
                            "kind": "record",
                            "timestamp": 4,
                            "record": {"type": "operationFinished", "operationId": "operation-1", "outcome": "completed", "completion": "normal", "finalEntryId": "answer-1"},
                        }
                    ]
                )
                return "done"
                ...
                self.assertEqual(
                    await cli.run_cli(
                        [
                            ...
                            "--plugin",
                            "read",
                            JSON_LIFECYCLE_CONTRACT["prompt"],
                        ]
                    ),
                    0,
                )
    ...
    async def test_plugin_selection_deduplicates_assembles_mcp_and_rejects_unknown_early(self):
        ...
        output = io.StringIO()
        with (
            patch.object(cli, "load_mcp_configs", return_value=[SimpleNamespace(alias="fixture")]),
    ...
    async def test_terminal_true_tty_entry_sets_raw_and_output_flags(self):
        ...
        current = [0, 0, 0, 0, 0, 0, []]
        with (
            patch.object(cli.sys.stdin, "fileno", return_value=9),
    ...
    async def test_terminal_edits_at_cursor_and_distinguishes_escape(self):
        ...
        terminal.tty = True
        try:
            os.write(write_fd, "你a\x1b[Db\x1b[C\x7f\r".encode())
            self.assertEqual(terminal.readline(""), "你b")
            os.write(write_fd, b"[D")
            self.assertEqual(terminal.escape_sequence(), b"D")
