...
class FramedHttpBody:
    ...
    async def __anext__(self) -> bytes:
        ...
        if self.decoded_bytes > self.max_body_bytes:
            raise RuntimeError(self.too_large_message)
        return chunk
    ...
    async def _read_exactly(self, size: int) -> bytes:
        try:
            return await asyncio.wait_for(self.reader.readexactly(size), remaining(self.deadline))
    ...
    async def _read_chunk(self) -> bytes:
        ...
        if size > self.max_body_bytes - self.decoded_bytes:
            raise RuntimeError(self.too_large_message)
        if size:
