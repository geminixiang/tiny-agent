...
class Terminal:
    def __init__(self):
        ...
        self.old = termios.tcgetattr(self.fd) if self.tty else None
    ...
    def display_position(text: str | list[str], columns: int) -> tuple[int, int]:
        ...
        for char in text:
            ...
            if cell == 2 and offset % columns == columns - 1:
                offset += 1
            offset += cell
    ...
    def redraw(self, prompt: str, line: list[str], cursor: int, old_row: int) -> int:
        ...
        if end_row > target_row:
            print(f"\x1b[{end_row - target_row}A", end="")
    ...
    def __enter__(self):
        if self.tty:
            ...
            current[1] |= termios.OPOST | termios.ONLCR
        ...
        while True:
            ...
            if char == b"\x1b":
                ...
                if key == b"D" and cursor:
                    cursor -= 1
                    while cursor and (unicodedata.combining(line[cursor]) or unicodedata.category(line[cursor]) in ("Cf", "Me")):
                        cursor -= 1
                    row = self.redraw(prompt, line, cursor, row)
                elif key == b"C" and cursor < len(line):
                    cursor += 1
                    while cursor < len(line) and (unicodedata.combining(line[cursor]) or unicodedata.category(line[cursor]) in ("Cf", "Me")):
                        cursor += 1
                    row = self.redraw(prompt, line, cursor, row)
                continue
            if char in (b"\r", b"\n"):
            ...
            if char in (b"\x08", b"\x7f"):
                if cursor:
                    start = cursor - 1
                    while start and (unicodedata.combining(line[start]) or unicodedata.category(line[start]) in ("Cf", "Me")):
                        start -= 1
                    del line[start:cursor]
                    cursor = start
                    row = self.redraw(prompt, line, cursor, row)
                continue
            if char >= b" ":
                pending += char
                if not complete_utf8(pending):
                    continue
                text = pending.decode(errors="replace")
                line[cursor:cursor] = text
                cursor += len(text)
                pending.clear()
                row = self.redraw(prompt, line, cursor, row)
