...
class SessionV2Corruption(Exception):
...
def fail(code: str, line: int, seq: int | None = None, message: str | None = None):
    raise SessionV2Corruption(code, line, seq, message)
...
def configuration(value: Any, line: int, seq: int) -> dict[str, Any]:
    ...
    if not isinstance(prompt_digest, str) or not DIGEST.fullmatch(prompt_digest):
        fail("INVALID_FACT", line, seq)
    environment = text(snapshot.get("environmentIdentity"), "INVALID_FACT", line, seq)
    ...
    for raw in snapshot["tools"]:
        ...
        tools.append({
            ...
            "replay": replay,
            "replayKey": text(tool.get("replayKey"), "INVALID_FACT", line, seq),
        })
    return {"model": model, "systemPromptDigest": prompt_digest, "tools": tools, "environmentIdentity": environment}
    ...
    if entry_type == "message":
        ...
        if message["role"] == "user":
            ...
            if state["reserved"].get(fact["id"]) != "toolResult":
                fail("INVALID_REFERENCE", line, seq)
            exact(entry, ["type", "stepId", "message", "toolName", "toolStartedId", "isError"], "INVALID_FACT", line, seq)
            step_id = uuid(entry.get("stepId"), "INVALID_FACT", line, seq)
            started_id = uuid(entry.get("toolStartedId"), "INVALID_FACT", line, seq)
            if not isinstance(entry.get("isError"), bool):
                fail("INVALID_FACT", line, seq)
...
def apply_record(state: dict[str, Any], fact: dict[str, Any], line: int):
    ...
    if kind == "runStarted":
        ...
        exact(record, ["type", "operationId", "operationKind", "inputThroughEntryId", "resultEntryId"], "INVALID_FACT", line, seq)
        ...
        if number == 1:
            ...
            if len(prior) != 1 or first["attempt"] != 1 or first["closed"] or first["failed"] or first["kind"] != step_kind or first["operationId"] != operation_id or first["configurationDigest"] != digest:
                fail("INVALID_TRANSITION", line, seq)
            first["closed"] = True
        attempt = {"operationId": operation_id, "stepId": step_id, "attemptId": attempt_id, "attempt": number, "kind": step_kind, "closed": False, "failed": False, "configurationSnapshot": snapshot, "configurationDigest": digest}
        state["attempts"][attempt_id] = attempt
        state["steps"][step_id] = [*prior, attempt]
        found["latestStepId"] = step_id
        state["operation"]["step"] = {"operationId": operation_id, "stepId": step_id, "attemptId": attempt_id, "attempt": number, "stepKind": step_kind, "status": "attempting", "configurationSnapshot": snapshot, "configurationDigest": digest}
        ...
        exact(record, ["type", "operationId", "stepId", "assistantEntryId", "toolIndex", "toolCallId", "toolName", "arguments", "replay", "replayKey", "resultEntryId"], "INVALID_FACT", line, seq)
        operation_id, found = get_operation(state, record.get("operationId"), line, seq)
        operation = state["operation"]
        ...
        if not declaration or declaration["replay"] != replay or declaration["replayKey"] != replay_key:
            fail("INVALID_TRANSITION", line, seq)
        result_id = reserve(state, record.get("resultEntryId"), line, seq, "toolResult")
        tool = {"operationId": operation_id, "toolStartedId": fact["id"], "stepId": uuid(record.get("stepId"), "INVALID_FACT", line, seq), "assistantEntryId": assistant_id, "toolIndex": index, "toolCallId": call_id, "toolName": tool_name, "arguments": arguments, "replay": replay, "replayKey": replay_key, "resultEntryId": result_id, "status": "pending"}
...
def finish_operation(state: dict[str, Any], record: dict[str, Any], line: int, seq: int):
    exact(record, ["type", "operationId", "operationKind", "outcome", "finalEntryId", "error"], "INVALID_FACT", line, seq)
    operation_id, found = get_operation(state, record.get("operationId"), line, seq)
    ...
    if outcome == "completed":
        ...
        if found["kind"] == "run":
            if not info or info.get("operationId") != operation_id or info["entry"].get("type") != "message" or info["entry"].get("stopReason") != "stop":
                fail("INVALID_REFERENCE", line, seq)
            ...
            if message["role"] != "assistant" or not (message.get("content") or "").strip() or any(tool["status"] == "pending" for tool in operation["toolCalls"]):
                fail("INVALID_TRANSCRIPT", line, seq)
        else:
...
def apply_usage(state: dict[str, Any], fact: dict[str, Any], line: int):
    ...
    for key in ("input", "output", "cacheRead", "cacheWrite"):
        state["usage"][key] += safe_int(usage.get(key), "INVALID_FACT", line, seq)
...
def validate_header(value: Any, line: int) -> dict[str, Any]:
    ...
    return {"id": uuid(header.get("id"), "INVALID_HEADER", line), "createdAt": safe_int(header.get("createdAt"), "INVALID_HEADER", line), "cwd": text(header.get("cwd"), "INVALID_HEADER", line), "provider": text(header.get("provider"), "INVALID_HEADER", line), "model": text(header.get("model"), "INVALID_HEADER", line)}


def reduce_session_v2(data: bytes) -> dict[str, Any]:
    ...
    state = {"header": header, "transcript": [], "activeContext": [], "usage": {"input": 0, "output": 0, "cacheRead": 0, "cacheWrite": 0}, "operation": {"kind": "idle"}, "repairedLength": last_lf + 1, "nextSeq": 1, "ids": set(), "reserved": {}, "entries": {}, "records": {}, "operations": {}, "attempts": {}, "steps": {}, "tools": {}, "toolPairs": set()}
