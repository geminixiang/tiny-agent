...
def _assistant(state):
    ...
    if operation["kind"] != "run" or not settled:
        return None
...
def plan_recovery(state: dict, current: dict) -> dict:
    ...
    if operation["kind"] == "idle":
        return {"type": "finish", "outcome": "completed", "completion": "normal"}
        ...
        if assistant:
            ...
            for index, call in enumerate(assistant["calls"]):
                ...
                if tool and tool["status"] == "pending":
                    results.append(_synthetic(assistant["assistantEntryId"], index, call, "interrupted", tool))
                elif not tool:
                    results.append(_synthetic(assistant["assistantEntryId"], index, call, "aborted"))
            if results:
                return {"type": "appendSynthetic", "results": results}
        return {"type": "finish", "outcome": "aborted"}
    if not operation.get("step"):
        return {
            "type": "startStep",
            "stepKind": "assistant" if operation["kind"] == "run" else "compaction",
            "attempt": 1,
            "contextThroughEntryId": operation.get("inputEntryId", operation.get("inputThroughEntryId")),
        }
    step = operation["step"]
    if step["status"] == "failed":
        return {"type": "finish", "outcome": "failed", "error": {"code": "model_error", "message": "provider request failed"}}
    if step["status"] == "attempting":
        if step["attempt"] == 2:
            return {"type": "blocked", "reason": "attempts_exhausted"}
    ...
    processed = {(tool["assistantEntryId"], tool["toolIndex"]) for tool in operation.get("toolCalls", [])}
    for index, call in enumerate(assistant["calls"]):
        if (assistant["assistantEntryId"], index) in processed:
            continue
        declaration = next((tool for tool in current["tools"] if tool["name"] == call["function"]["name"]), None)
        if not declaration:
            return {"type": "appendSynthetic", "results": [_synthetic(assistant["assistantEntryId"], index, call, "unknownTool")]}
        args = _json_object(call["function"].get("arguments"))
        if args is None:
            return {"type": "appendSynthetic", "results": [_synthetic(assistant["assistantEntryId"], index, call, "invalidArguments")]}
    ...
    if not declaration or not declaration["definitionDigest"]:
        return {"type": "blocked", "reason": "configuration_changed"}
