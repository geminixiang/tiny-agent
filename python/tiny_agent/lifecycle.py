...
class ExecutionLifecycle:
    ...
    def committed(self, facts: list[dict]) -> None:
        try:
            self._committed(facts)
        except Exception:
            pass

    def close(self) -> None:
        now = _timestamp()
        for attempt_id in list(self.models):
            self._complete_model(attempt_id, now, "effect_unknown")
    ...
    def _observe(self, event: dict) -> None:
        ...
        if kind == "session.attached":
            ...
            if not isinstance(operation_id, str) or operation_kind not in ("run", "compaction"):
                return
            ...
            if not self.session_id or not isinstance(event.get("toolStartedId"), str):
                return
    ...
    def _committed(self, facts: list[dict]) -> None:
        ...
        for fact in facts:
            ...
            if usage is None or not isinstance(attempt_id, str):
                continue
            transaction_usage[attempt_id] = usage
            self.usage[attempt_id] = usage
            operation_id = fact.get("operationId")
            if isinstance(operation_id, str):
                self._add_operation_usage(operation_id, usage)
        for fact in facts:
            self._fact(fact, transaction_usage)
    ...
    def _fact(self, fact: dict, transaction_usage: dict[str, dict]) -> None:
        ...
        if fact.get("kind") != "entry" or not isinstance(entry, dict):
            return
        if entry.get("type") == "message":
            ...
            return
        if entry.get("type") != "compaction" or not isinstance(entry.get("operationId"), str):
            return
        ...
        if attempt_id:
            self._complete_model(attempt_id, timestamp, "succeeded", self.usage.get(attempt_id))
    ...
    def _record(self, fact: dict, record: dict, timestamp: str) -> None:
        ...
        if kind == "stepAttempt":
            ...
            if not all(isinstance(value, str) for value in (operation_id, step_id, attempt_id)) or not isinstance(attempt, int):
                return
        ...
        if kind == "stepFailed":
            ...
            if not all(isinstance(value, str) for value in (operation_id, step_id, attempt_id)):
                return
        ...
        if kind == "toolStarted":
            ...
            if not all(isinstance(value, str) for value in values) or record.get("replay") not in ("safe", "never"):
                return
        ...
        if kind == "abortRequested":
            ...
            event = {
                ...
                "sessionId": self.session_id,
                "operationId": operation_id,
                "operationKind": record.get("operationKind"),
                "phase": record.get("phase"),
                "recovery": self.operations.get(operation_id, {}).get("recovery", False),
            }
            if isinstance(record.get("toolCallId"), str):
                event["toolCallId"] = record["toolCallId"]
            self._publish(event)
            return
        if kind != "operationFinished" or not isinstance(record.get("operationId"), str):
            return
        ...
        event = {
            ...
            "durationMs": _duration(operation["startedAt"], timestamp),
            "outcome": outcome,
        }
        if record.get("completion") in ("normal", "truncated"):
            event["completion"] = record["completion"]
        answer = self.answers.get(record.get("finalEntryId"))
        if answer:
            event["answer"] = answer[1]
        if operation_id in self.operation_usage:
            event["usage"] = self.operation_usage[operation_id]
        error = record.get("error", {})
        if isinstance(error.get("code"), str):
            event["errorType"] = error["code"]
        if isinstance(error.get("message"), str):
            event["errorMessage"] = error["message"]
        self._publish(event)
        self.operations.pop(operation_id, None)
        self.operation_usage.pop(operation_id, None)
    ...
    def _complete_model(self, attempt_id: str, timestamp: str, outcome: str, usage: dict | None = None, error_type: str | None = None) -> None:
        ...
        if not self.session_id or not attempt:
            return
        event = {
            ...
            "operationKind": attempt["kind"],
            "stepId": attempt["stepId"],
            "attemptId": attempt_id,
            "recovery": attempt["recovery"],
            "durationMs": _duration(attempt["startedAt"], timestamp),
            "outcome": outcome,
        }
        if usage:
            event["usage"] = _with_cache_rate(usage)
        if isinstance(error_type, str):
            event["errorType"] = error_type
        self.usage.pop(attempt_id, None)
        self._publish(event)
...
def _usage(value: object) -> dict | None:
    if not isinstance(value, dict) or any(not isinstance(value.get(key), (int, float)) for key in _empty_usage()):
        return None
...
def _fact_timestamp(fact: dict) -> str:
    return datetime.fromtimestamp(fact.get("timestamp", 0) / 1000, timezone.utc).isoformat(timespec="milliseconds").replace("+00:00", "Z")
...
def _duration(started: float, timestamp: str) -> float:
    return max(0, _time(timestamp) - started)
