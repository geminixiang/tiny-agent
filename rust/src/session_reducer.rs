...
pub struct ConfigurationSnapshot {
    ...
    pub output_options_digest: String,
}
...
pub struct StepState {
    ...
    pub settled_entry_id: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct SessionCorruption {
    pub code: CorruptionCode,
    pub line: usize,
    pub seq: Option<u64>,
}

impl std::fmt::Display for SessionCorruption {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{} at line {}", self.code.as_str(), self.line)
    }
}

impl std::error::Error for SessionCorruption {}

type Result<T> = std::result::Result<T, SessionCorruption>;
...
fn fail<T>(code: CorruptionCode, line: usize, seq: Option<u64>) -> Result<T> {
    Err(SessionCorruption { code, line, seq })
}
...
fn object(
    ...
    value
        .as_object()
        .cloned()
        .ok_or(SessionCorruption { code, line, seq })
}
...
fn canonical_string(value: &str) -> Result<String> {
    serde_json::to_string(value).map_err(|_| SessionCorruption {
        code: CorruptionCode::InvalidFact,
        line: 0,
        seq: None,
    })
...
fn configuration(value: &Value, line: usize, seq: u64) -> Result<ConfigurationSnapshot> {
    ...
    let adapter_identity = string(
        snapshot.get("adapterIdentity"),
        CorruptionCode::InvalidFact,
        line,
        Some(seq),
    )?;
    let routing_identity = string(
        snapshot.get("routingIdentity"),
        CorruptionCode::InvalidFact,
        line,
        Some(seq),
    )?;
    let output_options_digest = string(
        snapshot.get("outputOptionsDigest"),
        CorruptionCode::InvalidFact,
        line,
        Some(seq),
    )?;
    if !valid_digest(&output_options_digest) {
        return fail(CorruptionCode::InvalidFact, line, Some(seq));
    }
    let raw_tools = snapshot
        .get("tools")
        .and_then(Value::as_array)
        .ok_or(SessionCorruption {
            code: CorruptionCode::InvalidFact,
            line,
            seq: Some(seq),
        })?;
    ...
    for raw in raw_tools {
        ...
        let definition_digest = string(
            tool.get("definitionDigest"),
            CorruptionCode::InvalidFact,
            line,
            Some(seq),
        )?;
        if !valid_digest(&definition_digest) {
            return fail(CorruptionCode::InvalidFact, line, Some(seq));
        }
...
fn configuration_digest(snapshot: &ConfigurationSnapshot) -> Result<String> {
    let value = serde_json::to_value(snapshot).map_err(|_| SessionCorruption {
        code: CorruptionCode::InvalidFact,
        line: 0,
        seq: None,
    })?;
...
fn apply_message_entry(
    ...
    if role == "user" {
            ...
            && step.attempt_id == attempt_id
        {
            ...
            step.settled_entry_id = Some(fact_id.to_string());
            step.stop_reason = entry
                .get("stopReason")
                .and_then(Value::as_str)
                .map(str::to_string);
        }
        ...
        let result = object(
            entry.get("result").unwrap_or(&Value::Null),
            CorruptionCode::InvalidFact,
            line,
            Some(seq),
        )?;
...
fn apply_compaction_entry(
    ...
    let attempt_id = current_attempt.ok_or(SessionCorruption {
...
fn apply_tool_started(
    ...
    let tool = ToolCallState {
        ...
        environment_identity: string(
            record.get("environmentIdentity"),
            CorruptionCode::InvalidFact,
            line,
            Some(seq),
        )?,
...
fn validate_completed_operation(
    ...
    let info = state.entries.get(&final_id).ok_or(SessionCorruption {
        code: CorruptionCode::InvalidReference,
        line,
        seq: Some(seq),
    })?;
...
fn validate_header(value: &Value, line: usize) -> Result<SessionHeader> {
    ...
    Ok(SessionHeader {
        ...
        environment_identity: string(
            header.get("environmentIdentity"),
            CorruptionCode::InvalidHeader,
            line,
            None,
        )?,
...
impl JsonScanner<'_> {
    ...
    fn hex_quad(&mut self) -> Result<u16> {
        ...
        self.index = end;
        u16::from_str_radix(value, 16).map_err(|_| SessionCorruption {
            code: CorruptionCode::MalformedJson,
            line: self.line,
            seq: None,
        })
...
fn parse_line(source: &[u8], line: usize) -> Result<Value> {
    ...
    let source = std::str::from_utf8(source).map_err(|_| SessionCorruption {
        code: CorruptionCode::MalformedJson,
        line,
        seq: None,
    })?;
    scan_json(source, line)?;
    serde_json::from_str(source).map_err(|_| SessionCorruption {
        code: CorruptionCode::MalformedJson,
        line,
        seq: None,
    })
...
pub fn reduce_session(bytes: &[u8]) -> Result<SessionState> {
